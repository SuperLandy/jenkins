node {
	...
	stage('package and scan'){
        withSonarQubeEnv(credentialsId: 'sonarqube') {
            def POM_PATH = "project/service-zuul/pom.xml"
	...
	stage('docker building'){
		def SERVER_PORT = 8769
		def IMAGE = "192.168.1.172/hqj-dev/service-zuul:v1.k8s.${BUILD_NUMBER}"
		def BUILD_ARG = "--build-arg JAR_FILE=project/service-zuul/target/service-zuul.jar --build-arg SERVER_PORT=${SERVER_PORT} ."
				...
				sshPublisherDesc(configName: 'fz_server_175', 
					transfers: [
						sshTransfer(cleanRemote: false,
							execCommand: '''
                                IMAGE=192.168.1.172/hqj-dev/service-zuul:v1.k8s.${BUILD_NUMBER} ;\
                                source .bashrc ;\
                                kubectl set image deployment/hqj-service-zuul hqj-service-zuul="$IMAGE" --record -n wwdt ;\
                                kubectl rollout status deployment/hqj-service-zuul -n wwdt
							''', 
							execTimeout: 1200000
						)
					]
