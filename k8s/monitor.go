node {
    ...
    stage('package and scan'){
        withSonarQubeEnv(credentialsId: 'sonarqube') {
            def POM_PATH = "project/monitor/pom.xml"
	...
	stage('docker building'){
		def SERVER_PORT = 8090
		def IMAGE = "192.168.1.172/hqj-dev/monitor:v1.k8s.${BUILD_NUMBER}"
		def BUILD_ARG = "--build-arg JAR_FILE=project/monitor/target/monitor.jar --build-arg SERVER_PORT=${SERVER_PORT} ."
				...
				sshPublisherDesc(configName: 'fz_server_175', 
					transfers: [
						sshTransfer(cleanRemote: false,
							execCommand: '''
                                IMAGE=192.168.1.172/hqj-dev/monitor:v1.k8s.${BUILD_NUMBER} ;\
                                source .bashrc ;\
                                kubectl set image deployment/hqj-monitor hqj-monitor="$IMAGE" --record -n wwdt ;\
                                kubectl rollout status deployment/hqj-monitor -n wwdt
							''', 
							execTimeout: 1200000
						)
					]
