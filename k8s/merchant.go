node {
    ...
    stage('package and scan'){
        withSonarQubeEnv(credentialsId: 'sonarqube') {
            def POM_PATH = "project/merchant/pom.xml"
...
//    }
	...
	stage('docker building'){
		def SERVER_PORT = 8131
		def IMAGE = "192.168.1.172/hqj-dev/merchant:v1.k8s.${BUILD_NUMBER}"
		def BUILD_ARG = "--build-arg JAR_FILE=project/merchant/target/merchant.jar --build-arg SERVER_PORT=${SERVER_PORT} ."
				...
				sshPublisherDesc(configName: 'fz_server_175', 
					transfers: [
						sshTransfer(cleanRemote: false,
							execCommand: '''
                                IMAGE=192.168.1.172/hqj-dev/merchant:v1.k8s.${BUILD_NUMBER} ;\
                                source .bashrc ;\
                                kubectl set image deployment/hqj-merchant hqj-merchant="$IMAGE" --record -n wwdt ;\
                                kubectl rollout status deployment/hqj-merchant -n wwdt
							''', 
							execTimeout: 1200000
						)
					]
